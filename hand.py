...
class Hand:
    ...
    def suppressMotion(self, handLms, inverted=True, xScale=2, yScale=2, handOffsetX=400, handOffsetY=400):
        ...
        if inverted:
            # inverted for camera facing you
            self.handX = self.display[0] - self.handX
        ...
        if self.suppressShake:
            ...
            self.handPositions = self.queue(
                self.handPositions, [self.handX, self.handY])
    ...
    def controlMouse(self):
        if self.fingersRaised[1:5] == [1,1,1,1]:# if 4 fingers, thumb not included, are raised
            if self.mouseDown == False:
                ...
                self.mouseDown = True
        elif self.fingersRaised[1:5] == [0,1,1,1]: # if 3 fingers, thumb not included and index not included, are raised
            if self.mouseDown == False:
            ...
            elif self.fingersRaised[1:3] == [1, 1]:
    ...
    def draw(self):
        sucesss, img = self.cap.read()  # tuple of boolean success and image feed
        ...
        cv2.putText(img, str(fps), (10, 20), cv2.FONT_HERSHEY_SIMPLEX,
